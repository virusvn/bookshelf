...
class MediaModel {
  ...
  MediaModel(domain, result) {
    _id = int.parse(result["media"]["id"]);
    _name = result["media"]["name"];
    _domain = domain;
    _image = domain + result["media"]["image"];
    ...
    _image_heading = domain + result["media"]["image_heading"];
  ...
  List<Magazine> magazinesFromJson(result) {
    ...
    for (int i = 0; i < result["response"]["magazines"].length; i++) {
      Magazine magazine = Magazine(_domain, result["response"]["magazines"][i]);
